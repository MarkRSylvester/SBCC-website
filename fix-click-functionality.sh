#!/bin/zsh
# Script to fix the click functionality issue in the modal accordion

echo "===== Fixing Click Functionality ====="
...
# Update the accordion click handler
echo "Updating accordion click handler..."
...
echo "✅ Updated JS file with fixed click handler"

# Update test page to reflect improvements
TEST_FILE="modal-test/test-modal.html"
if [[ -f "$TEST_FILE" ]]; then
  # Add version timestamp
  TIMESTAMP=$(date +%s)
  sed -i '' "s|src=\"[^\"]*montecito-modal.js[^\"]*\"|src=\"../public/js/montecito-modal.js?v=$TIMESTAMP\"|g" "$TEST_FILE"
  echo "✅ Updated test file to use new JS version"
else
  echo "⚠️ Test file not found at: $TEST_FILE"
fi

# Add debugging button to test page
if [[ -f "$TEST_FILE" ]]; then
  # Check if debugging button already exists
  if ! grep -q "id=\"debug-accordion\"" "$TEST_FILE"; then
    # Add debugging button before the </body> tag
    sed -i '' 's|</body>|  <button id="debug-accordion" style="margin-top: 20px; background-color: #333; color: white; padding: 10px; border: none; border-radius: 4px; cursor: pointer;">Debug Accordion State</button>\n  <script>\n    document.getElementById("debug-accordion").addEventListener("click", function() {\n      if (window.sbcc && window.sbcc.logAccordionState) {\n        window.sbcc.logAccordionState();\n        window.sbcc.detectConflicts();\n      } else {\n        console.error("Debug functions not available");\n      }\n    });\n  </script>\n</body>|' "$TEST_FILE"
    echo "✅ Added debugging button to test page"
  else
    echo "✅ Debugging button already exists"
  fi
fi

echo "\n===== Fix Complete ====="
echo "Please refresh your test page and try the accordion functionality again."
echo "For debugging, open your browser console and click the 'Debug Accordion State' button."
